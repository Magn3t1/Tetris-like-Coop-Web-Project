...
class GameModel {
	...
	*/
	addClient(id, nickname){
		...
		this.ioSendMessage("SERVER", "Number of players : " + this.clients.size );
	...
	*/
	removeClient(id){
		...
		let kickedNickname = this.clientsNickname.get(id);
		...
		this.ioSendMessage("SERVER", kickedNickname + " disconnected.");
		this.ioSendMessage("SERVER", "Number of players : " + this.clients.size );

		if(this.mvc.gameState === 1){
			trace("GAME OVER");
...
module.exports = GameMVC; // export gameMVC class
